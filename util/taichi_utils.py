    ...
    for i in range(0, 2):
        for j in range(0, 2):
            for k in range(0, 2):
                ...
                partial_x += 1./dx * \
                    (value * dN_1(x_p_x_i) * N_1(y_p_y_i) * N_1(z_p_z_i))
                partial_y += 1./dx * (value * N_1(x_p_x_i)
                                      * dN_1(y_p_y_i) * N_1(z_p_z_i))
                partial_z += 1./dx * (value * N_1(x_p_x_i)
                                      * N_1(y_p_y_i) * dN_1(z_p_z_i))
...
@ti.func
def interp_2(vf, p, dx, BL_x=0.5, BL_y=0.5, BL_z=0.5):
    u_dim, v_dim, w_dim = vf.shape
    ...
    for i in range(-1, 3):
        for j in range(-1, 3):
            for k in range(-1, 3):
                ...
                value = sample(vf, iu + i, iv + j, iw + k)
                partial_x += 1./dx * \
                    (value * dN_2(x_p_x_i) * N_2(y_p_y_i) * N_2(z_p_z_i))
                partial_y += 1./dx * (value * N_2(x_p_x_i)
                                      * dN_2(y_p_y_i) * N_2(z_p_z_i))
                partial_z += 1./dx * (value * N_2(x_p_x_i)
                                      * N_2(y_p_y_i) * dN_2(z_p_z_i))
                interped += value * N_2(x_p_x_i) * N_2(y_p_y_i) * N_2(z_p_z_i)
...
def interp_u_MAC(u_x, u_y, u_z, p, dx):
    u_x_p = interp_2(u_x, p, dx, BL_x=0.0, BL_y=0.5, BL_z=0.5)
    u_y_p = interp_2(u_y, p, dx, BL_x=0.5, BL_y=0.0, BL_z=0.5)
    u_z_p = interp_2(u_z, p, dx, BL_x=0.5, BL_y=0.5, BL_z=0.0)
    return ti.Vector([u_x_p, u_y_p, u_z_p])

@ti.func
def interp_w_MAC(u_x, u_y, u_z, p, dx):
    u_x_p = interp_2(u_x, p, dx, BL_x=0.5, BL_y=0.0, BL_z=0.0)
    u_y_p = interp_2(u_y, p, dx, BL_x=0.0, BL_y=0.5, BL_z=0.0)
    u_z_p = interp_2(u_z, p, dx, BL_x=0.0, BL_y=0.0, BL_z=0.5)
    return ti.Vector([u_x_p, u_y_p, u_z_p])
